...
def recheck():
    ...
    for i in range(A_LENGTH):
        for j in range(B_LENGTH):
            for k in range(H_HEIGHT):
                if getForce(i, j, k, 0) < 2.16074 * (B_LENGTH - j) / B_LENGTH + 0.8 * round(pow(EXPEND_RATE, header[i, k]), 3) + 0.0005 * rainAffected * pow(EXPEND_RATE, H_HEIGHT - k):
                    ...
                    if legal(i, j, k) and getForce(i, j, k, 1) < 2.16074 * round(pow(DECREASE_RATE, j - header[i, k] + 1), 3):
                        model[i, j, k] = True
                        deleter[i, j] = deleter[i, j] + 1
